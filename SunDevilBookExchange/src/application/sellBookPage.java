...
public class sellBookPage extends CycledView{
	...
	void createGUI() {
		...
		ImageView bookImg = new ImageView(new Image("placeholder.png"));
		...
		list_book_btn.setOnAction(actionEvent -> {
			...
			try {
				...
				if(isLikeNew) {
					newBook.price = newBook.price * 9 / 10;
				}
				if(isModerate) {
					newBook.price = newBook.price * 75 / 100;
				}
				if(isHeavily) {
					newBook.price = newBook.price / 2;
				}
		...
		getChildren().addAll(Title, Sell, titleField, ISBNField, authorField, ConditionRect, Condition, line, likeNew, moderatelyUsed, heavilyUsed, synopsis, uploadImage, bookImg
